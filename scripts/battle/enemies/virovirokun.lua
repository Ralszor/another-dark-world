...
local function getViroConfig(key, merge)
    if Mod.libs and Mod.libs.virovirokun then
        return Kristal.getLibConfig("virovirokun", key, merge)
    end
end
...
function Virovirokun:init()
    ...
    if getViroConfig("enable_cook") then
    ...
    if getViroConfig("enable_quarantine") then
...
function Virovirokun:onActStart(battler, name)
    local sprite_lookup = getViroConfig("take_care_sprites", true) or {}
    local offset_lookup = getViroConfig("take_care_offsets", true) or {}
